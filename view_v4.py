...
from controler_v4 import Controler
import sys
...
class MyTableWidget(QWidget):
    ...
    def __init__(self, parent, controler):
        ...
        self.tab3 = Bourse_tab(self,controler)

        # Add tabs
        self.tabs.addTab(self.tab1, "Information compte")
        self.tabs.addTab(self.tab2, "Budget")
        self.tabs.addTab(self.tab3, "Bourse")
...
class ActionWidget(QWidget):
    def __init__(self, parent, controler):
        ...
        self.button_ajouterdepense_csv = QPushButton("Fichier CSV depenses")
        ...
        layoutV1.addWidget(self.button_ajouterdepense_csv)
        ...
        self.button_ajouterdepense_csv.clicked.connect(self.read_csv_depenses)
    ...
    def read_csv_depenses(self):
        file_path = QFileDialog.getOpenFileName(self, 'Choisir fichier csv')
        self.controler.read_csv_depenses(file_path)
...
class ActionBudgetWidget(QWidget):
    def __init__(self, parent, controler):
        ...
        # Boutton pour observer les budgets
        self.button_observer_budgets = QPushButton('Caractéristiques budgets')
        self.button_observer_budgets.clicked.connect(self.observation_budgets)
        self.button_observer_budgets.setMaximumHeight(20)
        ...
        layout.addWidget(self.button_observer_budgets)
    ...
    def observation_budgets(self):
        self.fenetre_temp = Observation_Budgets(None, self.controler)
        self.fenetre_temp.setWindowModality(Qt.ApplicationModal)
        self.fenetre_temp.setWindowFlags(Qt.FramelessWindowHint)
        self.fenetre_temp.show()
...
class Observation_Budgets(QWidget):
    def __init__(self, parent, controler):
        super().__init__(parent)
        self.controler = controler
        # self.setFixedSize(300, 300)
        ...
        # Widgets avec les caractéristiques des budgets
        self.caract_budget=[]
        for i in self.controler.compte.budgets:
            self.caract_budget.append(CellBudget(self,i.categorie, i.somme, i.frequence, i.debut))

        # Button pour sortir
        self.button_sortir = QPushButton("Retour")
        self.button_sortir.clicked.connect(self.sortir)
        self.button_sortir.setMaximumHeight(20)

        #########  Creation Layouts #########
        main_layout = QVBoxLayout()

        ####### Orgnasiation Layouts ########
        # main_layout.addWidget(self.log_window)
        for i in self.caract_budget:
            main_layout.addWidget(i)
        main_layout.addWidget(self.button_sortir)
...
class CellBudget(QFrame):
    def __init__(self, parent, categorie, montant, frequence, debut):
        super().__init__(parent)
        self.setFixedSize(650,60)
        self.setFrameStyle(QFrame.Panel | QFrame.Sunken)
        self.setLineWidth(2)
        # widgets
        self.label_categorie = QLabel("<b>Catégorie:</b> {}".format(categorie))
        self.label_montant = QLabel("<b>Montant:</b> {} EUR".format(montant))
        self.label_frequence = QLabel("<b>Frequence:</b> <i>{}</i>".format(frequence))
        self.label_debut = QLabel("<b>Debut:</b> {}".format(debut))

        # layouts
        layoutG = QGridLayout()
        layoutG.addWidget(self.label_categorie,0,0)
        layoutG.setAlignment(self.label_categorie, Qt.AlignLeft)
        layoutG.addWidget(self.label_montant,0,1)
        layoutG.setAlignment(self.label_montant, Qt.AlignLeft)
        layoutG.addWidget(self.label_frequence,1,0)
        layoutG.setAlignment(self.label_frequence, Qt.AlignLeft)
        layoutG.addWidget(self.label_debut,1,1)
        layoutG.setAlignment(self.label_debut, Qt.AlignLeft)
        layoutG.setColumnStretch(0,1)
        layoutG.setColumnStretch(1,1)
        layoutG.setContentsMargins(2,2,2,2)
        layoutG.setSpacing(0)
...
#        self.setMaximumWidth(500)
    ...
    def drawgraph(self):
        ...
        date_debut=self.controler.periode_obs_dep[0]
        date_fin=self.controler.periode_obs_dep[1]
        budget=budget_periode_donnee(self.controler.compte,date_debut,date_fin)
        depenses=[]
        ...
        for i in range(1,n_groups): # range(1,n_groups) pour ne pas compter la catégorie 'salaire'
            ...
            depenses.append(somme_depense if somme_depense>0 else
                                  -somme_depense)
        ...
        if len(index)!=len(budget):
            for i in range(abs(len(index)-len(budget))):
                budget.append(0)

        ax.bar(index, budget, bar_width, alpha=opacity,
                         color='b', label='Budget')
        for i, v in enumerate(budget):
            ax.text(i-0.25,v + 3, str(v), color='blue', fontweight='bold')

        ax.bar(index + bar_width, depenses, bar_width,
                         alpha=opacity, color='g', label='Dépenses')
        for i, v in enumerate(depenses):
        ...
        ax.set_xticklabels(categorie_list[1:],fontsize=8)
        ax.legend()
...
class logWidget(QTextEdit):
    def __init__(self, parent, controler):
        ...
        self.append('Date         Somme (€)         Commentaire \n')
        for i in depenses_triees[0]:
            self.append(dt.datetime.strftime(i.date,'%d/%m/%Y')+f'   {abs(i.somme)}         {i.commentaire} \n')
    ...
    def refresh(self):
        ...
        self.append('Date              Somme (€)         commentaire  \n')
        for i in depenses_triees[0]:
            self.append(dt.datetime.strftime(i.date,'%d/%m/%Y')+f'   {i.somme}                  {i.commentaire} \n')
...
#        self.setMaximumWidth(300)
    ...
    def drawgraph(self):
        ...
        try:
            mean=np.ones(len(sommes))*sommes[-1]/len(sommes)
        except:
            mean=[]

        ax.plot(dates,sommes,'ro')
        ax.plot(dates,sommes,'r',label='Somme')
        ax.plot(dates,mean,'b--',label='Moyenne')
        ax.xaxis.set_ticks(range(0,len(dates),int(len(dates)/7)+1))
...
#################################################### TAB: Bourse #######################################################
########################################################################################################################
class Bourse_tab(QWidget):
    def __init__(self, parent, controler):
        super().__init__(parent)
        self.setMinimumSize(900, 500)

        # widgets
        self.affichage_widget = AffichageBourseWidget(self, controler)
        # Creation layouts
        main_layout = QHBoxLayout()

        # Organisation layouts
        main_layout.addWidget(self.affichage_widget)

        self.setLayout(main_layout)

########################################################################################################################
class AffichageBourseWidget(QWidget):
    def __init__(self, parent, controler):
        super().__init__(parent)
        self.controler = controler
        self.controler.addClient(self)
        ...
        # Information sur bourse
        self.info_bourse = QLabel("")
        self.info_bourse.setFrameStyle(QFrame.Panel | QFrame.Raised)
        self.info_bourse.setLineWidth(3)
        self.info_bourse.setFixedSize(350, 250)

        # Choisir la période d'observation
        self.label_deb = QLabel("Début:")
        self.label_fin = QLabel("Fin:")
        self.date_deb = QDateEdit(QDate(2010,1,1))
        self.date_fin = QDateEdit(QDate(2019,12,31))
        self.date_fin.setMaximumDate(QDate.currentDate())
        # connecter signal
        self.date_deb.dateChanged.connect(self.date_debut_changed)
        self.date_fin.dateChanged.connect(self.date_fin_changed)

        # Menu deroulant avec les noms des CAC 40
        self.label_menu_deroulant=QLabel('Entreprise:')
        self.CAC_40_names = QComboBox()
        self.CAC_40_names.addItems(self.controler.get_CAC_40_names())

        # Boutton pour ajouter un budget
        self.button_bourse = QPushButton("Ok")
        self.button_bourse.clicked.connect(self.open_bourse)
        self.button_bourse.setMaximumHeight(20)

        # Organisation layout
        sub_layout_h1.addWidget(self.label_menu_deroulant)
        sub_layout_h1.addWidget(self.CAC_40_names)

        sub_layout_h2.addWidget(self.label_deb)
        sub_layout_h2.addWidget(self.date_deb)

        sub_layout_h3.addWidget(self.label_fin)
        sub_layout_h3.addWidget(self.date_fin)


        sub_layout_v.addWidget(self.info_bourse)
        sub_layout_v.addLayout(sub_layout_h1)
        sub_layout_v.addLayout(sub_layout_h2)
        sub_layout_v.addLayout(sub_layout_h3)
        sub_layout_v.addWidget(self.button_bourse)
    ...
    def drawgraph(self):
        dates,prix,volume = self.controler.historique_bourse(self.date_deb.date().toString("dd/MM/yyyy"),self.date_fin.date().toString("dd/MM/yyyy"))

        # Affichage evolution prix
        ax1 = self.figure.add_subplot(211)
        ax1.clear()
        ax1.set_title("Evolution du prix", fontsize=10)

        ax1.plot(dates,prix,'b')
        ax1.xaxis.set_ticks(range(0,len(dates),int(len(dates)/7)+1))
        ax1.xaxis.set_ticks(range(0,len(dates),int(len(dates)/7)+1))
        ...
        # Affichage evolution volule
        ax2 = self.figure.add_subplot(212)
        ax2.clear()
        ax2.set_title("Evolution du prix", fontsize=10)

        ax2.plot(dates,volume,'b')
        ax2.xaxis.set_ticks(range(0,len(dates),int(len(dates)/7)+1))
        ax2.xaxis.set_ticks(range(0,len(dates),int(len(dates)/7)+1))
    ...
    def open_bourse(self):
        nom=self.CAC_40_names.currentText()
        self.controler.read_csv_bourse(self.controler.compte,nom)

    def refresh(self):
        self.drawgraph()
        date_debut=dt.datetime(self.date_deb.date().year(),self.date_deb.date().month(),self.date_deb.date().day())
        date_fin=dt.datetime(self.date_fin.date().year(),self.date_fin.date().month(),self.date_fin.date().day())

        dep,budget_inv=self.controler.bourse_budget_restant(self.controler.compte,date_debut,date_fin)
        rest=abs(budget_inv)-abs(dep)
        invest=self.controler.get_invest_budget(self.controler.compte)

        try:
            text_info_budget ="<b>Budget Investissment</b><br/><i>Montant {}  €</i><br/> Fréquence: {} </i><br/> Debut: {}<br/><i>".format(invest.somme,invest.frequence,invest.debut.date())
        except:
            text_info_budget ="<b>Budget Investissment</b><br/><i>Montant {}  €</i><br/> Fréquence: {} </i><br/> Debut: {}<br/><i>".format("None","None","None")
        text_depenses_investissement = "<br/><i><b>Budget Investissment pour la période considerée</b><br/><i>Montant {} EUR </b><br/><i>Depenses Investissment {}  €</i><br/> Budget restant: {}".format(abs(budget_inv),abs(dep),rest)
        self.info_bourse.setText(text_info_budget + text_depenses_investissement)
