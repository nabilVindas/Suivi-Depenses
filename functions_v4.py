...
CAC_40_names=['ACCOR','AIRBUS','AIR LIQUIDE','ARCELORMITTAL SA','ATOS','AXA','BNP PARIBAS ACT.A',
'BOUYGUES','CAPEGEMINI','CARREFOUR','CREDIT AGRICOLE','DANONE','DASSAULT SYSTEMES',
'ENGIE','ESSILORLUXOTTICA','HERMES INTL','KERING','LEGRAND',"L''OREAL",'LVMH','MICHELIN','ORANGE',
'PERNOD RICARD','PEUGEOT','PUBLICIS GROUPE SA','RENAULT','SAFRAN','SAINT GOBAIN','SANOFI',
'SCHNEIDER ELECTRIC','SOCIETE GENERALE','SODEXO','STMICROELECTRONICS','TECHNIPFMC',
'THALES','TOTAL','UNIBAIL-RODAMCO-WE','VEOLIA ENVIRON.','VINCI','VIVENDI']
...
class donnee_bourse:
    ...
    def init_value(self,nom,prix,volume,date):
        self.nom=nom
        self.prix=prix
        self.volume=volume
        self.date=date
...
# Determiner les budgets pour une période donnee_bourse
def budget_periode_donnee(compte,date_debut,date_fin):
    budgets_periode=[]
    for i in compte.budgets:
        s=0
        if i.frequence=='Jour':
            d=1
        elif i.frequence=='Semaine':
            d=7
        elif i.frequence=='Mois':
            d=30
        else:
            d=365

        date_inter=i.debut
        while date_inter<date_debut:
            date_inter=date_inter+dt.timedelta(days=d)
        if date_inter<date_fin:
            while date_inter<date_fin:
                s=float(i.somme)+s
                date_inter=date_inter+dt.timedelta(days=d)
        budgets_periode.append(s)
    try:
        budgets_periode.pop(0)
    except:
        pass
    return(budgets_periode)
...
def read_csv_bourse(compte,name):
    # compte.bourse=[]
    with open(f'bourse/{name}_quote_chart.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        bourse_inter=donnee_bourse()
        bourse_inter.init_value(name,[],[],[])
        i=0
        for row in readCSV:
            if i>=1:
                bourse_inter.date.append(row[0])
                bourse_inter.prix.append(float(row[1]))
                bourse_inter.volume.append(float(row[2]))
            else:
                i=i+1
        # return(bourse_inter)
        compte.bourse=bourse_inter
        # compte.bourse.append(bourse_inter)

# Renvoie seulement les informations de bourse correspondant a une période donnee
def bourse_date_donnee(dates, prix,volume, date_debut, date_fin):
    list_date=dates
    list_prix=prix
    list_volume=volume
    date_debut = dt.datetime.strptime(date_debut,"%d/%m/%Y")
    ...
    indices = []
    for i in range(len(list_date)):
        d = dt.datetime.strptime(list_date[i],"%Y-%m-%d %H:%M")
        if (d<date_debut or d>date_fin):
            indices.append(i)
    k=0
    for i in indices:
        ...
        del(list_volume[i-k])
        k=k+1
    return (list_date,list_prix,list_volume)

def bourse_budget_restant(compte,date_debut,date_fin):
    s=0
    for i in compte.depenses:
        if i.categorie=="Investissement":
            date_inter=i.date
            if date_inter>date_debut and date_inter<date_fin:
                s=float(i.somme)+s
    ...
    for i in compte.budgets:
        if i.categorie=="Investissement":
            ...
            if date_inter<date_fin:
                while date_inter<date_fin:
                    budget_invest=budget_invest+i.somme
                    date_inter=date_inter+dt.timedelta(days=d)
    return(s,budget_invest)
...
def test_final():
    ...
    try:
        ...
        if((float(depense_test.somme) == float(montant1)) &
    ...
    # Test 18: Determiner les budgets pour une periode donnee
    nb_test += 1
    date_debut=dt.datetime(2019,10,1)
    date_fin=dt.datetime.now()
    try:
        budget_periode_test=budget_periode_donnee(compte_test,date_debut,date_fin)
        print("Test 18 (budget for the consider period): SUCCED")
        nb_test_succed += 1
    except:
        print("Test 18 (budget for the consider period): FAILED <system>")

    # Test 19: Budget d'investissement restant à depenser pour une periode donnée
    nb_test += 1
    date_debut=dt.datetime(2019,10,1)
    date_fin=dt.datetime.now()
    try:
        budget_invest_periode_test=bourse_budget_restant(compte_test,date_debut,date_fin)
        print("Test 19 (investement budget for the consider period): SUCCED")
        nb_test_succed += 1
    except:
        print("Test 19 (investement budget for the consider period): FAILED <system>")

    # Test 20: Lecture fichier CSV avec les données bourse d'une entreprise
    nb_test += 1
    try:
        read_csv_bourse(compte_test,"ACCOR")
        print("Test 20 (read CSV file for stock marcket): SUCCED")
        nb_test_succed += 1
    except:
        print("Test 20 (read CSV file for stock marcket): FAILED <system>")


    # Test 21: Données de bourse pour une période donnée pour une entreprise donnée
    nb_test += 1
    date_debut="01/01/2019"
    date_fin="31/12/2019"
    try:
        bourse_date_donnee(compte_test.bourse.date, compte_test.bourse.prix,compte_test.bourse.volume, date_debut, date_fin)
        print("Test 21 (stock market information for a consider period of time): SUCCED")
        nb_test_succed += 1
    except:
        print("Test 21 (stock market information for a consider period of time): FAILED <system>")
